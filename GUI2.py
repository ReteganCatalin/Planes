...
class GUI:
    ...
    def ShowPlaneMatrix(self):
        ...
        def DrawAtPosition(index_row, index_column, main_frame , value):
            def CreateRectangle(draw_index_row=index_row, draw_index_column=index_column, frame=main_frame , draw_value=value):
               GUI.color="white"
               canvas = tkinter.Canvas(frame,height=32, width=32)
               canvas.grid(row=draw_index_row, column =draw_index_column, sticky = tkinter.N+tkinter.E+tkinter.S+tkinter.W)
               if draw_value==1:
        ...
        CreateBoard(place_planes_frame)
        self.place_planes.mainloop()
    ...
    def StartHitting(self):
        ...
        def create_board(main_frame):
            for index_row in range(8):
                for index_column in range(17):
                    if index_column<8:
                        DrawHitBoardAtPosition(index_row, index_column, main_frame, player_hit_matrix[index_row][index_column])
                    elif index_column==8:
                        DrawHitBoardAtPosition(index_row, index_column, main_frame, 3)
                    else:
                        DrawHitBoardAtPosition(index_row, index_column, main_frame, computer_hit_matrix[index_row][index_column-9])
        def DrawHitBoardAtPosition(index_row, index_column, main_frame , value ):
            def Create(draw_index_row=index_row, draw_index_column=index_column, master=main_frame , draw_value=value):
               GUI.color="green"
               ...
               if draw_value==2:
                   ...
                   canvas.create_rectangle(2, 2, 30, 30, fill="grey")
               return
            return Create()
